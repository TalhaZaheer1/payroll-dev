...
async function ensureGlobals() {
  ...
  if (!g) {
    g = await GlobalsModel.create({ currentPayPeriod: null, autoCreatePayPeriods: false });
  } else if (typeof g.autoCreatePayPeriods !== "boolean") {
    // keep default false if not set
    await GlobalsModel.findByIdAndUpdate(g._id, { $set: { autoCreatePayPeriods: !!g.autoCreatePayPeriods }});
  }
...
async function setAutoCreation(req, res, next) {
  try {
    ...
    const g = await ensureGlobals();
    await GlobalsModel.findByIdAndUpdate(g._id, { $set: { autoCreatePayPeriods: enabled } });
    res.json({ enabled });
  } catch (e) {
    next(e);
  }
...
async function checkAndAutoCreateIfNeeded() {
  const globals = await ensureGlobals();
  if (!globals?.autoCreatePayPeriods) return { created: false, reason: "auto disabled" };
